...
select substr(hiredate,1,2) 입사년, substr(hiredate,4,2) 입사월 from employee;

-- 2. SUBSTR 함수를 사용하여 4월에 입사한 사원을 출력 하시오.
select * from tab;
select ename 사원이름, substr(hiredate,4,2) 입사월 from employee where substr(hiredate,4,2) = 04;

-- 3. MOD 함수를 사용하여 직속상관이 홀수인 사원만 출력하시오.

--select ename 사원이름, mod

select ename, manager from employee where MOD (manager , 2) = 1; 

-- 4. MOD 함수를 사용하여 월급이 3의 배수인 사원들만 출력하세요.

select ename 사원이름, salary 월급 from employee where mod(salary,3) = 0;

-- 5. 입사한 년도는 2자리 (YY), 월은 (MON)로 표시하고 요일은 약어 (DY)로 지정하여 출력 하시오.

--select hiredate 입사일, to_date(hiredate, 'YY-MON-DY') from employee;

select hiredate , to_char ( hiredate, 'YY MON DD DY') as 출력결과 from employee; 
...
-- 9. 사원들의 상관 사번을 출력하되 상관이 없는 사원에 대해서는 null 값대신 0으로 출력 하시오.
-- 오라클은 방번호가 1로 시작
...
select rpad(substr(eno,1,2),4,'*') as "가린번호",
